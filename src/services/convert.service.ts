...
export class ConvertService {
  ...
  public static async convertForEInk(image: Jimp, w: number, h: number, format: "bmp" | "png"): Promise<Buffer> {
    const cropped = image.cover(w, h);
    cropped.bitmap = this.floydSteinberg(cropped.bitmap);

    if (format === "png") {
      return cropped.getBufferAsync("image/png");
    } else if (format === "bmp") {
  ...
  public static convertFormat(fromMime: string, fromBuffer: Buffer): Promise<Jimp> {
    try {
      switch (fromMime) {
        case "image/heic":
          return (heicDecoder({ buffer: fromBuffer }) as Promise<Bitmap>).then(
            (res) =>
  ...
  private static floydSteinberg(image: Bitmap): Bitmap {
    ...
    for (let currentPixel = 0; currentPixel <= imageDataLength; currentPixel += 4) {
      ...
      if (xpos < w - 1) saveUpdateImageData(currentPixel + 4, err * 7); // pixel to the right
      if (xpos > 0 && ypos < h - 1) saveUpdateImageData(currentPixel + 4 * w - 4, err * 3); // pixel to the bottom left
      if (ypos < h - 1) saveUpdateImageData(currentPixel + 4 * w - 0, err * 5); // pixel below
      if (ypos < h - 1 && xpos < w - 1) saveUpdateImageData(currentPixel + 4 * w + 4, err * 1); // pixel to the bottom right
